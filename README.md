...
cd backend
mvn clean package
java -jar target/reminot-1.0.12.jar
```
...
Готовый файл:
- `backend/target/reminot-1.0.12.jar`
...
```powershell
cd backend
.\build-exe.ps1 -AppVersion 1.0.12
```
...
```powershell
.\build-exe.ps1 -AppVersion 1.0.12 -IconPath "..\docs\media\my-icon.ico"
```
...
```bash
git add .
git commit -m "Prepare release v1.0.12"
